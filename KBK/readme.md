## Today 
<pre>
    1. 이미지 업로드 파일명 겹쳤을 때 중복파일을 업로드하는 경우
    2. admin 로그인 시 회원 수정 hidden
    3. index 카테고리별 정렬
</pre>
...
<pre>
  ...
  4. Tab1 이름 추가 完
  5. 삭제 버튼 추가 完
  6. 회원 정보(Member). 레시피(recipe) 테이블 추가
  7. 레시피 페이지 Tab Menu에 삭제 수정 버튼 추가 후 기능 구현완료(수정페이지 미구현)
</pre>
<pre>
    1. SNS 로그인 추가
...
<pre>  
  ...
  3. 로그인, 로그아웃 구현 완료
  4. 레시피 업로드 테스트 성공, 업로드한 레시피 List 뿌리기 성공
  5. 추가한 레시피 삭제 성공
  6. 재료 JSON 추가 방식 수정함
  7. 레시피 이미지 파일 업로드
</pre>
...
#### 25-11-24
<pre>
  1. recipeUpdate 레이아웃 작성 완료 및 구현 완료
</pre>

#### 25-11-25
<pre>
  1. 레시피 수정(재료, 조리방법) : 추가버튼 클릭하지않으면 삭제도 안됨
